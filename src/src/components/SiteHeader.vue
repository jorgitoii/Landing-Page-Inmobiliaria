  ...
  <header :class="['site-header', { visible: headerVisible }]">
    <div class="header-inner">
      ...
      <nav class="header-nav">
        <a href="#" @click.prevent="scrollTo('location')">Ubicacion</a>
        <a href="#" @click.prevent="openGallery">Galeria</a>
...
.brand-main   { font-family: var(--font-serif); font-size: 12px; font-weight: 400; letter-spacing: 0.3em; color: var(--color-white); }
.brand-sub    { font-family: var(--font-jp); font-size: 9px; font-weight: 200; letter-spacing: 0.2em; color: var(--color-accent); margin-top: 3px; }
.header-nav   { display: flex; align-items: center; gap: 40px; }
.header-nav a { font-family: var(--font-serif); font-size: 11px; font-weight: 400; letter-spacing: 0.28em; color: rgba(200,225,240,0.65); text-decoration: none; text-transform: uppercase; transition: color 0.3s ease; position: relative; }
.header-nav a::after { content: ""; position: absolute; bottom: -3px; left: 0; width: 0; height: 1px; background: var(--color-accent); transition: width 0.3s ease; }
.header-nav a:hover { color: var(--color-accent); }
.header-nav a:hover::after { width: 100%; }
.nav-contact { border: 1px solid rgba(122,180,212,0.35); padding: 8px 22px; color: var(--color-accent); }
.nav-contact::after { display: none; }
.nav-contact:hover { background: rgba(122,180,212,0.08); }
</style>
